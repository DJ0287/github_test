...
Public Class Task04RecvPLC_01sec
    Inherits Task00Base
    ...
    Public Function Run() As TaskEventArgs Implements Task00Interface.Run
        ...
        If PLCINFO.Connect() = 0 Then
            Try
                Dim dsBit = datasource("IF_PLC_TO_SRS_BIT_CTR")
                Dim dsWord = datasource("IF_PLC_TO_SRS_WORD_CTR")
                ecode = PLCINFO.ReadBlockData(dsWord, dsBit)
                If ecode <> 0 Then
                    Throw New Exception
                End If
                RECVDATA.BIT_PLC_SRS_CTR.Updated()
                RECVDATA.WORD_PLC_SRS_CTR.Updated()
                ...
                If RECVDATA.BIT_PLC_SRS_CTR.No3_スケジュール更新 Or RECVDATA.BIT_PLC_SRS_CTR.No4_スケジュール更新 Then
                    ...
                    Select Case presended
                        ...
                        Case 4
                            ...
                            End If
                        Case Else
                            e.etype = (RECVDATA.BIT_PLC_SRS_CTR.No3_スケジュール更新 And Not RECVDATA.BIT_PLC_SRS_CTR.No4_スケジュール更新) * (-3) + _
                                      (Not RECVDATA.BIT_PLC_SRS_CTR.No3_スケジュール更新 And RECVDATA.BIT_PLC_SRS_CTR.No4_スケジュール更新) * (-4) + _
