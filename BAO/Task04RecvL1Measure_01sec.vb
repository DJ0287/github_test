...
Public Class Task04RecvL1Measure_01sec
    Inherits Task00Base
    ...
    Public Function Run() As TaskEventArgs Implements Task00Interface.Run
        ...
        If L1INFO.Connection = 0 Then
            ...
            If L1INFO.RecieveNewMeasure Then
                L1INFO.RecieveNewMeasure = False
                Try

                    Dim dsBit = datasource("IF_PLC_TO_SRS_BIT_M9000")
                    Dim dsWord = datasource("IF_PLC_TO_SRS_WORD_D6000")
                    Dim ifmap = L1INFO.L1MessageMap(3) ' Measurement Value
                    ...
                    If Not ifmap.GetBuffer() Is Nothing Then
                        For Each item In dsBit.interfacelist
                            item.SetField(ifmap.GetField(item.comment) > 0)
                        Next
                        For Each item In dsWord.interfacelist
                            item.SetField(Convert.ToUInt16(ifmap.GetField(item.comment) / item.unit))
                        Next
                    End If

                    RECVDATA.BIT_PLC_SRS_M9000.Updated()
                    RECVDATA.WORD_PLC_SRS_D6000.Updated()
