...
Public Class Frm02Base_BAO
    ...
    Private Sub Form2Base_Load(sender As Object, e As EventArgs) Handles MyBase.Load 'Handles MyBase.Load
        ...
        C1FlexGrid1.Col = 8
        C1FlexGrid1.set_ColWidth(8, 1200 * scaler)
        C1FlexGrid1.Text = "Sn" & vbCrLf & "Consumption"

        'C1FlexGrid1.Col = 10
        C1FlexGrid1.Col = 9
        C1FlexGrid1.set_ColWidth(9, 1000 * scaler)
        C1FlexGrid1.Text = "Control" & vbCrLf & "Completion time"

        'C1FlexGrid1.Col = 11
        C1FlexGrid1.Col = 10
        C1FlexGrid1.set_ColWidth(10, 1200 * scaler)
        C1FlexGrid1.Text = "Sn" & vbCrLf & "Necessary" & vbCrLf & "volume"
        ...
        C1FlexGrid1.set_ColWidth(11, C1FlexGrid1.Width - twidth)
        '&MSFlexGrid1.set_ColWidth(11, 1200)
        'C1FlexGrid1.Col = 12
        C1FlexGrid1.Col = 11
        C1FlexGrid1.Text = "Sn" & vbCrLf & "Dissolution"
        ...
        C1FlexGrid2.Text = "Sn Dissolution" & vbCrLf & "Plan"
        ...
        C1FlexGrid2.Text = "Sn Dissolution" & vbCrLf & "completion"
        ...
        C1FlexGrid2.Text = "Sn dissolution" & vbCrLf & "margin"
        ...
        C1FlexGrid2.Text = "Sn dissolution" & vbCrLf & "compensation"
...
#End If
    ...
    Protected Sub FormRedraw(no)
        ...
        If CONDITION.LINESPEED.Display(CONDITION.LINESPEED.Check(0, ライン速度(0)), Task03Control_01sec.scantime) Then
            SetLineText(LB_ETL_RUN, LineConditionType.ETL運転中)
        Else
            SetLineText(LB_ETL_RUN, LineConditionType.ETL停止中)
        End If
        ...
        '    Case 0
        '        If RECVDATA.BIT_PLC_SRS_IND.No3_調整材 Then
        '            SetLineText(LB_ETL_RUN, LineConditionType.調整材, "")
        '        Else
        '            If CONDITION.LINESPEED.Display(0, Task03Control_01sec.scantime) Then
        '                SetLineText(LB_ETL_RUN, LineConditionType.ETL運転中, "")
        '            Else
        ...
        For j = 4 To 23
            If Products.Count > 0 Then
                If Products.Count > j - 4 Then
                    ...
                    C1FlexGrid1.set_TextMatrix(j, 0, Products(j - 4).No)
                    'If j = 4 Then
                    'LS = ライン速度(0)
                    'C1FlexGrid1.set_TextMatrix(j, 1, Format(Products(j - 4).Weight_bk, "0.000"))
                    'Else
                    LS = Products(j - 4).TPH.ToDouble() * 1000 ^ 2 / 60 / 7.85 / Products(j - 4).Thickness.ToDouble() / Products(j - 4).Width.ToDouble()
                    C1FlexGrid1.set_TextMatrix(j, 1, Format(Products(j - 4).Weight.ToDouble(), "0.000"))
                    'End If
                    'Dim L = Products(j - 4).Weight.ToDouble() / 7.85 / (Products(j - 4).Thickness.ToDouble() * Products(j - 4).Width.ToDouble() * 0.000001)
                    C1FlexGrid1.set_TextMatrix(j, 2, Format(LS, "####0"))
                    C1FlexGrid1.set_TextMatrix(j, 3, Format(Products(j - 4).Thickness.ToDouble(), "0.000"))
                    C1FlexGrid1.set_TextMatrix(j, 4, Format(Products(j - 4).Width.ToDouble(), "####0"))
                    ...
                    C1FlexGrid1.set_TextMatrix(j, 8, Format(Products(j - 4).Sn, "#0.00"))
                    If Products(j - 4).Te <> 0 Then
                        C1FlexGrid1.set_TextMatrix(j, 9, "Te" & Math.Abs(Products(j - 4).Te))
                        teend = Products(j - 4).Te
                    Else
                        C1FlexGrid1.set_TextMatrix(j, 9, "")
                    End If
                    C1FlexGrid1.set_TextMatrix(j, 10, Format(Products(j - 4).S, "##0.00"))
                    If j = 4 Or Products(j - 4).Te > 0 Then
                        C1FlexGrid1.set_TextMatrix(j, 11, Format(Products(j - 4).S1, "##0.00"))
                    Else
                        C1FlexGrid1.set_TextMatrix(j, 11, "")
                    End If
                Else
                    If Products.Count > 0 And Products.Count = j - 4 Then
                        Dim lastsch = Products(Products.Count - 1)
                        C1FlexGrid1.set_TextMatrix(j, 0, "")
                        C1FlexGrid1.set_TextMatrix(j, 1, Format(0.0, "0.000"))
                        C1FlexGrid1.set_TextMatrix(j, 2, Format(0.0, "####0"))
                        C1FlexGrid1.set_TextMatrix(j, 3, Format(0.0, "0.000"))
                        C1FlexGrid1.set_TextMatrix(j, 4, Format(0.0, "####0"))
                        ...
                        C1FlexGrid1.set_TextMatrix(j, 8, Format(0.0, "##0.00"))
                        C1FlexGrid1.set_TextMatrix(j, 9, "Te" & teend + 1)
                        C1FlexGrid1.set_TextMatrix(j, 10, Format(0.0, "##0.00"))
                        C1FlexGrid1.set_TextMatrix(j, 11, "")
                    Else
                        For K = 0 To 11
                ...
                End If
            Else
                If schProducts.Count > j - 4 Then
                    C1FlexGrid1.set_TextMatrix(j, 0, Format(schProducts(j - 4).No, "0000"))
                    C1FlexGrid1.set_TextMatrix(j, 1, Format(schProducts(j - 4).Weight, "0.000"))
                    C1FlexGrid1.set_TextMatrix(j, 2, Format(schProducts(j - 4).LS, "####0"))
                    C1FlexGrid1.set_TextMatrix(j, 3, Format(schProducts(j - 4).Thickness, "0.000"))
                    C1FlexGrid1.set_TextMatrix(j, 4, Format(schProducts(j - 4).Width, "####0"))
                    C1FlexGrid1.set_TextMatrix(j, 5, Format((schProducts(j - 4).Coating_Bot + schProducts(j - 4).Coating_Top) * 1000, "####0"))
                    For K = 6 To 11
                        C1FlexGrid1.set_TextMatrix(j, K, "")
                    Next
                Else
        ...
        C1FlexGrid1.set_TextMatrix(3, 9, Format(ETLPRODSCHS(no).TE0, "###0.0"))
        C1FlexGrid1.set_TextMatrix(3, 10, Format(ETLPRODSCHS(no).Sf, "###0.0"))
...
#If 0 And DEBUG Then
        For j = 4 To 23
            C1FlexGrid1.set_TextMatrix(j, 0, "COIL" & j - 3)
            C1FlexGrid1.set_TextMatrix(j, 1, Format(0.0, "0.000"))
            C1FlexGrid1.set_TextMatrix(j, 2, Format(0.0, "####0"))
            C1FlexGrid1.set_TextMatrix(j, 3, Format(0.0, "0.000"))
            C1FlexGrid1.set_TextMatrix(j, 4, Format(0.0, "####0"))
            C1FlexGrid1.set_TextMatrix(j, 5, Format(0.0, "####0"))
            C1FlexGrid1.set_TextMatrix(j, 6, Format(0.0, "###0.0"))
            C1FlexGrid1.set_TextMatrix(j, 7, Format(0.0 + 0.0, "###0.0"))
            'C1FlexGrid1.set_TextMatrix(j, 8, Format(0.0, "##0.00"))
            C1FlexGrid1.set_TextMatrix(j, 8, Format(0.0, "##0.00"))
            C1FlexGrid1.set_TextMatrix(j, 9, "Te" & 0 + 1)
            C1FlexGrid1.set_TextMatrix(j, 10, Format(0.0, "##0.00"))
